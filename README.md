# 20250502上課筆記
## QRcode
![qrcode](https://hackmd.io/_uploads/Hkh023Wlee.png)
---
...
## 第五條指令
利用createGraphics指令，產撐一個與video視訊畫面一樣的寬高，一樣的大小內容，該圖片顯示在視訊畫面的上方
...
function drawOverlayGraphics() {
  overlayGraphics.background(0); // 設定背景為黑色
  for (let i = 0; i < overlayGraphics.width; i += 20) { // 每隔 20 為一個單位
    for (let j = 0; j < overlayGraphics.height; j += 20) {
      let col = capture.get(i, j); // 擷取 capture 對應位置的顏色
      overlayGraphics.fill(col); // 設定圓的顏色
      overlayGraphics.noStroke();
      overlayGraphics.ellipse(i + 10, j + 10, 15, 15); // 繪製寬高為 15 的圓
    }
  }
}
```
